            ...
            <div class="col-lg-8">
                ...
                <div class="position-relative mb-3">
                    <div class="image-container position-relative">
                        ...
                        $imageToDisplay = (file_exists(FCPATH . $imagePath) && !empty($wisata['foto'])) ? base_url($imagePath) : $defaultImage;
                        ?>
                        ...
                        <?php if (!empty($wisata['sumber_foto'])): ?>
                            <div class="sumber-foto position-absolute">
                                <p class="mb-0">Sumber Foto : <?= esc($wisata['sumber_foto']); ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="bg-white border border-top-0 p-4">
                        ...
                        <div class="mb-2">
                            <span class="badge badge-primary text-uppercase font-weight-semi-bold p-2 mr-2">
                                <?= esc($wisata['nama_kategori_wisata'] ?? 'Kategori Tidak Diketahui') ?>
                            </span>
                        </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="mb-3">
                    ...
                    <div class="bg-white border border-top-0 p-3">
                        <?php if (!empty($wisataToDisplay)) : ?>
                            <?php foreach (array_slice($wisataToDisplay, 0, 5) as $wisataLainnya) : ?>
                                <div class="d-flex align-items-center bg-white mb-3" style="height: 110px; object-fit: cover;">
                                    <?php
                                    $defaultImage = base_url('assets-baru/img/error_logo.webp');
                                    $imagePath = 'asset-user/uploads/foto_wisata/' . ($wisataLainnya['foto'] ?? '');
                                    $imageToDisplay = (file_exists(FCPATH . $imagePath) && !empty($wisataLainnya['foto'])) ? base_url($imagePath) : $defaultImage;
                                    ?>

                                    <img class="img-fluid lazyload" style="width: 110px; height: 110px; object-fit: cover;"
                                         src="<?= esc($imageToDisplay) ?>"
                                         alt="<?= esc($wisataLainnya['nama_wisata']) ?>"
                                         loading="lazy">

                                    <div class="w-100 h-100 px-3 d-flex flex-column justify-content-center border border-left-0">
                                        <div class="mb-1 mt-3">
                                            <span class="badge badge-primary text-uppercase font-weight-semi-bold p-1 mr-2" style="font-size: 11px;">
                                                <?= esc($wisataLainnya['nama_kategori_wisata'] ?? 'Kategori Tidak Diketahui') ?>
                                            </span>
                                        </div>

                                        <h6 class="m-0 text-secondary font-weight-bold mt-1">
                                            <?= esc($wisataLainnya['nama_wisata']) ?>
                                        </h6>

                                        <?php
                                        $deskripsiBersih = html_entity_decode(strip_tags($wisataLainnya['deskripsi'] ?? ''));
                                        $deskripsiSingkat = strlen($deskripsiBersih) > 40 ? substr($deskripsiBersih, 0, 40) . '...' : $deskripsiBersih;
                                        ?>
...
<div class="col-lg-12 text-center mt-5 mb-4">
    <h6 class="slogan text-uppercase font-weight-bold mb-4">
        Detail Peta - <?= esc($wisata['nama_wisata']) ?>
    </h6>
</div>
    ...
    wisataMarker.bindPopup(`
        <div style="text-align: center;">
            <img 
                ...
                alt="<?= $lang === 'en' ? $wisata['nama_wisata_eng'] : $wisata['nama_wisata'] ?>" 
            ...
            /><br>
            <strong><?= $lang === 'en' ? $wisata['nama_wisata_eng'] : $wisata['nama_wisata'] ?></strong><br>
