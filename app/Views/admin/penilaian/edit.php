...
<div class="container px-4">
    <h4 class="mb-4">Edit Penilaian</h4>
    ...
    <form action="<?= base_url('admin/penilaian/update/'.$penilaian['id_penilaian']) ?>" method="post">
        <?= csrf_field() ?>
            ...
            <select name="id_alternatif" id="id_alternatif" class="form-select" required>
                ...
                <?php foreach($alternatif as $alt): ?>
                    <option value="<?= $alt['id_alternatif'] ?>"
                        <?= ($penilaian['id_alternatif'] == $alt['id_alternatif']) ? 'selected' : '' ?>>
                        <?= esc($alt['nama_wisata']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <?php
            $selected_subs = json_decode($penilaian['subkriteria_json'] ?? '{}', true);
        ?>
        ...
        <?php foreach($kriteria as $k): ?>
            <div class="mb-4">
                        ...
                        <?php foreach($subs as $sub): ?>
                            <div class="form-check">
                                <input 
                                    ...
                                    <?= (isset($selected_subs[$k['id_kriteria']]) && in_array($sub['id_subkriteria'], $selected_subs[$k['id_kriteria']])) ? 'checked' : '' ?>
                                >
                ...
                <?php else: ?>
                    <select name="subkriteria[<?= $k['id_kriteria'] ?>]" class="form-select" required>
                        <option value="">-- Pilih Subkriteria --</option>
                        <?php foreach($subs as $sub): ?>
                            <option value="<?= $sub['id_subkriteria'] ?>"
                                <?= (isset($selected_subs[$k['id_kriteria']]) && $selected_subs[$k['id_kriteria']] == $sub['id_subkriteria']) ? 'selected' : '' ?>>
                                <?= esc($sub['nama_subkriteria']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
